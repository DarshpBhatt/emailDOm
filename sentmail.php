...
$sql = "SELECT id, sentTp, reg_date,subject ,message FROM SentEmail ORDER BY id DESC";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<br>Email UUID:". $row["id"]."<br> To: ". $row["sentTp"]. "<br> Sub:" . $row["subject"] . "<br><br> Message:<br>".$row['message']."<br><br> Sent at :".$row["reg_date"]."<p><hr>";
    }
} else {
    echo "0 results";
}
...
$sql = "DELETE FROM SentEmail WHERE id = $idtoDel	";
